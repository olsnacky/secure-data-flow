...
public /* partial */ class Graph implements NodeMap
{
    ...
    public void AddMethodCallNode(ASTNode callpoint, IMethodBinding method, Node recv, List<Node> args, Node tr, SubGraph subgraph)
    {
    ...
    public void AddDataFlowEdges(SrcNodes src, SrcNodes dest)
    {
        // done
        AddDataFlowEdges(src, dest.dataFlow);
    }

    public void AddDataFlowEdges(SrcNodes src, Node dest)
    {
        // done
    ...
    public void AddControlFlowEdges(SrcNodes src, Node dest)
    {
        // done
    ...
    public void AddControlFlowEdges(NodeSet src, Node dest)
    {
        for (Node s : src.GetEnumerator())
            AddControlFlowEdge(new ControlFlowEdge(s, dest, null));
    }

    public void AddControlFlowEdges(Node src, SrcNodes dest)
    {
        for (Node child : dest.controlFlow.GetEnumerator())
            AddControlFlowEdge(src, child);
    }

    public void AddControlFlowEdges(Node src, NodeSet dest)
    {
        for (Node child : dest.GetEnumerator())
            AddControlFlowEdge(src, child);
    }

    public void AddControlFlowEdges(Node src, org.eclipse.jdt.core.dom.Statement dest)
    {
        for (Node child : securedataflow.handlers.DataFlowVisitor.statements.get(dest).GetEnumerator())
            AddControlFlowEdge(src, child);
    }

    public void AddMethodCallNode(MethodInvocation callpoint, IMethodBinding method, Node recv, List<Node> args, Node tr, SubGraph subgraph)
    {
        MethodCall node = new MethodCall(callpoint, recv, method, args, tr);
        node.subgraph = subgraph;
        methods.add(node);
...
//            System.out.println("\t" + method.dotty() + ";");
            ...
            for (PointsToSameEdge edge : pointsToSameEdges)
                for (Map.Entry<IVariableBinding, FieldNode> f : edge.src.fields.entrySet())
                {
                    ...
                    if (edge.dest.fields.containsKey(field_name))
                        AddAliasEdge(new AliasEdge(f.getValue(), edge.dest.fields.get(field_name), Arrays.asList(edge)));
                }
                ...
                if (edge1.dest instanceof ValueNode)
                    for (Map.Entry<Node, AliasEdge> item2 : edge1.dest.aliasEdges.entrySet())
                    {
                        ...
                        for (DataFlowEdge edge3 : edge2_dest.dataFlowEdges.values())
                            AddDataFlowEdge(new DataFlowEdge(edge1.src, edge3.dest, Arrays.asList(edge1, edge2, edge3)));
                    }
                ...
                if (edge1.dest instanceof ValueNode)
                    for (Map.Entry<Node, AliasEdge> item2 : edge1.dest.aliasEdges.entrySet())
                    {
                        ...
                        for (ControlFlowEdge edge3 : edge2_dest.controlFlowEdges.values())
                            AddControlFlowEdge(new ControlFlowEdge(edge1.src, edge3.dest, Arrays.asList(edge1, edge2, edge3)));
                    }
    ...
    public SubGraph CloneMethodBody(MethodFoo method_body, MethodCall method_invocation)
    {
        ...
        for (MethodCall method : method_body.graph.methods)
            AddMethodCallNode(method.callpoint, method.method, method.recv == null ? null : map(method.recv), Substitute(method.args), map(method.return_value), clone);
